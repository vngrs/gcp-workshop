...
mysql_pass = os.environ.get('MYSQL_PASSWORD', '12345')
...
def init():
    try:
       db = MySQLdb.connect(mysql_host,mysql_user,mysql_pass)
       cursor = db.cursor()
       cursor.execute("DROP DATABASE IF EXISTS kubernetes")
       cursor.execute("CREATE DATABASE kubernetes")
       cursor.execute("USE kubernetes")
